import { generateDetectiveSitemapUrls, generateServiceSitemapUrls, generateSitemapXml, generateSitemapIndexXml, chunkSitemapUrls } from "./lib/sitemap.js";
...
const sitemapCache = new NodeCache({ stdTTL: 3600 }); // 1 hour cache
  ...
  app.get("/sitemap.xml", async (req: Request, res: Response) => {
    try {
      // Check cache
      let indexXml = sitemapCache.get("sitemap-index");
      if (!indexXml) {
        // Generate detective sitemap chunks
        const detectiveUrls = await generateDetectiveSitemapUrls();
        const detectiveChunks = chunkSitemapUrls(detectiveUrls);
        const detectiveFiles = detectiveChunks.map((_, i) => `detectives-${i + 1}.xml`);

        // Generate service sitemap chunks
        const serviceUrls = await generateServiceSitemapUrls();
        const serviceChunks = chunkSitemapUrls(serviceUrls);
        const serviceFiles = serviceChunks.map((_, i) => `services-${i + 1}.xml`);

        const allFiles = [...detectiveFiles, ...serviceFiles];
        indexXml = generateSitemapIndexXml(allFiles);
        sitemapCache.set("sitemap-index", indexXml);
        sitemapCache.set("detective-files", detectiveFiles);
        sitemapCache.set("service-files", serviceFiles);
      }
  ...
  // INDIVIDUAL SITEMAP FILE ROUTE
  app.get("/sitemaps/:file", async (req: Request, res: Response) => {
    try {
      const { file } = req.params;
      let xml = sitemapCache.get(file);
      if (!xml) {
        // Determine type
        if (file.startsWith("detectives-")) {
          const detectiveUrls = await generateDetectiveSitemapUrls();
          const chunks = chunkSitemapUrls(detectiveUrls);
          const idx = parseInt(file.split("-")[1]) - 1;
          if (chunks[idx]) {
            xml = generateSitemapXml(chunks[idx]);
            sitemapCache.set(file, xml);
          }
        } else if (file.startsWith("services-")) {
          const serviceUrls = await generateServiceSitemapUrls();
          const chunks = chunkSitemapUrls(serviceUrls);
          const idx = parseInt(file.split("-")[1]) - 1;
          if (chunks[idx]) {
            xml = generateSitemapXml(chunks[idx]);
            sitemapCache.set(file, xml);
      ...
      res.status(500).type("text/plain").send("Error generating sitemap file");
    }
  });
