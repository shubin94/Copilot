...
import { detectives, reviews, services } from "../../shared/schema.ts";
 ...
 * Returns limited data (top 10 for JSON-LD ItemList)
 */
export async function getLocationDetectivesForSEO(
  country: string,
  state?: string,
  city?: string
): Promise<Array<{ id: string; businessName: string | null; slug: string | null; city: string; state: string; country: string }>> {
  try {
    const generateSlug = (text: string): string => {
      if (!text) return "";
      return text
    ...
    // Match country (flexible: by code or name)
    ...
    const countryCode = countryCodeMap[countrySlug] || country.toUpperCase();

    // Add country filter
    conditions.push(eq(detectives.country, countryCode));

    // Add state filter if provided
    if (state) {
      conditions.push(eq(detectives.state, state));
    }

    // Add city filter if provided
    if (city) {
      conditions.push(eq(detectives.city, city));
    }

    // Query top 10 detectives for this location
    const rows = await db
      .select({
        id: detectives.id,
        businessName: detectives.businessName,
