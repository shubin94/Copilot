...
async function analyzeRoutes(): Promise<void> {
  ...
  for (const route of getRoutes) {
    ...
    const isProtected =
      handlerCode.includes("requireAuth") ||
    ...
    // Find JOINs
    const joins = [];
    if (handlerCode.includes("INNER JOIN")) joins.push("INNER JOIN");
    if (handlerCode.includes("LEFT JOIN")) joins.push("LEFT JOIN");
    if (handlerCode.includes("JOIN")) joins.push("JOIN");

    // Check caching
    let cacheStatus = "No cache";
    if (
      handlerCode.includes("Cache-Control") ||
    ...
    if (
      route.path.includes("/api/detectives") &&
      !route.path.includes("/:id")
    ) {
      estimatedSize = "50-200 KB";
      riskLevel = "High";
    } else if (
      route.path.includes("/api/services") &&
      !route.path.includes("/:id")
    ) {
  ...
  for (const route of highRisk.slice(0, 10)) {
    ...
    console.log(
      `   Issues: ${
        route.hasSelectStar ? "SELECT * | " : ""
      }${route.hasLimit ? "" : "No LIMIT | "}${route.joins.length > 0 ? `${route.joins.length} JOIN operations` : ""}`
    );
    console.log();
  }
  ...
  console.log(
    `Risk Distribution: ${summary.highRisk} HIGH | ${summary.mediumRisk} MEDIUM | ${summary.lowRisk} LOW`
  );
  console.log(
    `SELECT * Patterns: ${summary.withSelectStar} endpoints (${((summary.withSelectStar / summary.totalEndpoints) * 100).toFixed(1)}%)`
  );
  console.log(
    `Missing LIMIT clause: ${summary.withoutLimit} endpoints (${((summary.withoutLimit / summary.totalEndpoints) * 100).toFixed(1)}%)`
  );
  console.log(
    `With Caching: ${summary.cached} endpoints (${((summary.cached / summary.totalEndpoints) * 100).toFixed(1)}%)`
  );
  console.log(
    `Protected Endpoints: ${summary.protected} endpoints (${((summary.protected / summary.totalEndpoints) * 100).toFixed(1)}%)`
  );
