...
async function auditDatabase(): Promise<void> {
  ...
  try {
    ...
    for (const table of tableResults.rows) {
      ...
      const avgRowSize =
        rowCount > 0 ? Math.round(totalBytes / rowCount) : 0;
    ...
    if (rlsRes.rows.length > 0) {
      for (const row of rlsRes.rows) {
        console.log(
          `   ${row.tablename}: ${row.policy_count} RLS policies`
        );
      }
    } else {
    ...
    const indexByTable: { [key: string]: string[] } = {};
    ...
    const estimates = [
      {
        ...
        description:
          "20 detective cards with bio truncation, logo URLs, etc",
      },
      {
    ...
    console.log(
      "   Impact: 450+ KB per request with no pagination limits\n"
    );
    ...
    console.log(
      "   Impact: 3x data per service when fetching with all relationships\n"
    );
    ...
    console.log(
      "6. Lazy-load nested relationships instead of JOINing by default"
    );
