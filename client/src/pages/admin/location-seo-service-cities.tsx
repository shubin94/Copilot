...
export default function AdminLocationSeoServiceCities() {
  ...
  useEffect(() => {
    ...
    if (stateFilter) rows = rows.filter(r => r.state_slug === stateFilter);
    if (cityFilter) rows = rows.filter(r => r.city_slug === cityFilter);
    setFilteredRows(rows);
  }, [countryFilter, stateFilter, cityFilter, allRows]);

  const loadData = async () => {
    try {
      setIsLoading(true);
      const res = await api.get<{ success: boolean; data: Row[] }>(
        `/api/admin/location-seo/service-locations?level=city&category=${category}`
      );
      setAllRows(res.data || []);
  ...
  const statesForCountry = countryFilter
    ? Array.from(new Set(allRows.filter(r => r.country_slug === countryFilter).map(r => r.state_slug))).sort()
    : [];

  const citiesForState = (countryFilter && stateFilter)
    ? Array.from(new Set(
        allRows
          .filter(r => r.country_slug === countryFilter && r.state_slug === stateFilter)
          .map(r => r.city_slug)
      )).sort()
    : [];
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await api.post<{ success: boolean }>("/api/admin/service-location-seo", {
        ...
        state_slug: selectedRow.state_slug,
        city_slug: selectedRow.city_slug,
      ...
      toast({ title: "Success", description: "Service Location SEO saved successfully" });
      await loadData();
      setShowModal(false);
    } catch (err: any) {
      const msg = err?.message || "Failed to save service location SEO";
      setError(msg);
  ...
  return (
    <DashboardLayout role="admin">
      <div className="space-y-6">
        <div>
          <h1 className="text-3xl font-bold">Service Locations — Cities</h1>
          <p className="text-sm text-gray-500 mt-1">
            Manage SEO for pages like <code className="bg-gray-100 px-1 rounded">/locations/surveillance/india/karnataka/bengaluru/</code>
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <div>
            ...
            <select
              ...
              className="w-full px-3 py-2 border rounded focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:bg-gray-50 disabled:cursor-not-allowed"
            >
        ...
        {isLoading ? (
          ...
          <div className="text-center py-10 text-gray-500">No cities found</div>
        ) : (
            ...
            <table className="w-full min-w-max">
              ...
              <tbody className="divide-y">
                {filteredRows.map((row) => (
                  <tr key={`${row.category_slug}:${row.country_slug}:${row.state_slug}:${row.city_slug}`} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1">
                        ...
                        <a
                          href={`/locations/${row.category_slug}/${row.country_slug}/${row.state_slug}/${row.city_slug}/`}
                          target="_blank"
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-lg p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto">
              ...
              <p className="text-sm text-gray-500 mb-4">
                {serviceCategories.find(c => c.slug === selectedRow.category_slug)?.label || selectedRow.category_slug} — {selectedRow.country_slug} / {selectedRow.state_slug} / {selectedRow.city_slug}
              </p>
