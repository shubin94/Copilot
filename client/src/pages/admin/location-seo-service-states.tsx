...
export default function AdminLocationSeoServiceStates() {
  ...
  }, [countryFilter, stateFilter, allRows]);

  const loadData = async () => {
    try {
      setIsLoading(true);
      const res = await api.get<{ success: boolean; data: Row[] }>(
        `/api/admin/location-seo/service-locations?level=state&category=${category}`
      );
      setAllRows(res.data || []);
  ...
  return (
    <DashboardLayout role="admin">
      <div className="space-y-6">
        <div>
          <h1 className="text-3xl font-bold">Service Locations — States</h1>
          <p className="text-sm text-gray-500 mt-1">
            Manage SEO for pages like <code className="bg-gray-100 px-1 rounded">/locations/surveillance/india/karnataka/</code>
          </p>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="text-center py-10 text-gray-500">No states found</div>
        ) : (
            ...
            <table className="w-full min-w-max">
              ...
              <tbody className="divide-y">
                {filteredRows.map((row) => (
                  <tr key={`${row.category_slug}:${row.country_slug}:${row.state_slug}`} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1">
                        ...
                        <a
                          href={`/locations/${row.category_slug}/${row.country_slug}/${row.state_slug}/`}
                          target="_blank"
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-lg p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto">
              ...
              <p className="text-sm text-gray-500 mb-4">
                {serviceCategories.find(c => c.slug === selectedRow.category_slug)?.label || selectedRow.category_slug} — {selectedRow.country_slug} / {selectedRow.state_slug}
              </p>
