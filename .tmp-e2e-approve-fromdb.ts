...
async function run() {
  ...
  const email = `e2e.fromdb.${unique}@example.com`;
  const selectedCategories = ["Intercity Travels", "Cyber Security"];
  const passwordHash = await bcrypt.hash("Detective@12345", 10);

  const [app] = await db.insert(detectiveApplications).values({
    fullName: `E2E FromDB ${unique}`,
    email,
  ...
  await httpRequest(`${BASE_URL}/api/applications/${app.id}`, {
    method: "PATCH",
    headers: {
      "Content-Type": "application/json",
      "X-CSRF-Token": approveCsrf,
    },
    body: JSON.stringify({ status: "approved", reviewNotes: "E2E approval from DB test" }),
    credentials: "include",
  }, jar);

  const [detective] = await db.select().from(detectives).where(eq(detectives.contactEmail, email)).limit(1);
  if (!detective) throw new Error("Detective profile not found after approval");
